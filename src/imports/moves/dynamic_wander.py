...
class DynamicWander:
	def __init__(self, character, max_acceleration, wander_target, explicit_target, wander_offset=100.0, wander_radius=50.0, wander_rate=0.1):
	...
	def get_steering(self):
		...
		# colocar explicit target en la circunferencia del wander
		self.explicit_target.kinematic.position = self.wander_target.kinematic.position + self._random_binomial() * self.wander_radius * self.wander_target.kinematic.orientation_to_vector().rotate(90)
		# Usar Face para orientar al personaje hacia el target de wander
		face = Face(
			character=self.character,
			face_target=self.explicit_target,
			explicit_target=self.wander_target
		)
